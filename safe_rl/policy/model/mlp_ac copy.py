from safe_rl.util.torch_util import to_device, to_tensor
import numpy as np
import torch
...
class CholeskyGaussianActor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, action_low, action_high, hidden_sizes, activation):
        ...
        self.action_low = torch.nn.Parameter(to_tensor(action_low)[None, ...], requires_grad=False)  # (1, act_dim)
        self.action_high = torch.nn.Parameter(to_tensor(action_high)[None, ...], requires_grad=False)  # (1, act_dim)
        ...
        # print('action:',self.act_dim)
        # print('action*:',(self.act_dim * (self.act_dim + 1)) // 2)
        # quit()
